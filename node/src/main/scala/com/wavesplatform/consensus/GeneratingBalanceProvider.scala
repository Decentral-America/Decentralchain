...
object GeneratingBalanceProvider {
  ...
  def balance(blockchain: Blockchain, account: Address, blockId: Option[BlockId] = None): Long = {
    ...
    val depth  = if (height >= blockchain.settings.functionalitySettings.generationBalanceDepthFrom50To10000AfterHeight) SecondDepth else FirstDepth
